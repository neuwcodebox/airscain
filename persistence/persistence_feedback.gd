...
static func success_message(action: Action, repairs: Array[String]) -> String:
	...
	if used_backup and repaired_state_count > 0:
		return "%s 완료 · 이전 정상 저장을 복구하고 상태 %d건을 정리했습니다" % [display_action, repaired_state_count]
	if used_backup:
		return "%s 완료 · 이전 정상 저장을 복구했습니다" % display_action
	if repaired_state_count > 0:
		return "%s 완료 · 문제가 있던 상태 %d건을 자동으로 정리했습니다" % [display_action, repaired_state_count]
	return "%s 완료" % display_action
...
static func failure_message(action: Action, diagnostic: String, current_operation_preserved: bool = false) -> String:
	if diagnostic == SaveStore.ERROR_MISSING:
		return "저장된 작전을 찾을 수 없습니다."
	if diagnostic.begins_with("지원하지 않는 저장"):
		return "이 저장 파일은 현재 게임 버전과 호환되지 않습니다. 게임 버전을 확인해 주세요."
	if action == Action.SAVE:
		if _is_file_write_failure(diagnostic):
			return "저장 파일을 쓸 수 없습니다. 저장 공간을 확인한 뒤 다시 시도해 주세요."
		return "현재 작전을 안전하게 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."
	var message := "저장 파일이 손상되어 작전을 불러오지 못했습니다." if _is_damaged_save(diagnostic) else "저장된 작전을 불러오지 못했습니다."
	if diagnostic.contains("백업도 복원할 수 없습니다"):
		message = "기본 저장과 이전 정상 저장을 모두 불러오지 못했습니다."
	return "%s 현재 작전은 그대로 유지됩니다." % message if current_operation_preserved else "%s 새 작전을 시작하거나 다른 저장 파일을 사용해 주세요." % message

static func action_name(action: Action) -> String:
	return "저장" if action == Action.SAVE else "불러오기"
