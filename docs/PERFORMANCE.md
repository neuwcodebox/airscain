...
웹의 첫 발사·전투 효과 지연은 [웹 첫 등장 프레임 측정](PERFORMANCE_WEB_HITCH.md)에 별도로 정리했다.

현재 코드의 재측정과 병목별 비용은 [2026-09-10 난전 분석](PERFORMANCE_2026-09-10.md), 원본 수치는 [측정 데이터](PERFORMANCE_2026-09-10.json)에 정리했다. 구조 변경과 일반 최적화 기법의 적용 방식·우선순위·검증 기준은 [난전 최적화 후보](PERFORMANCE_OPTIMIZATION.md)를 참고한다. 채택한 변경과 전후 검증은 [최적화 구현 결과](PERFORMANCE_IMPLEMENTATION.md)에 정리한다. 아래는 이전 변경의 조건별 측정 기록이다.
