...
건물과 자산의 피해 연기는 실제 타격점의 작은 상자형 방출면에서 부드러운 radial alpha billboard를 연속적으로 내보낸다. 단일 emitter의 16초 수명, 좁은 확산각, 양의 상승 속도와 일정한 횡풍이 하나로 이어진 plume을 만들고 color ramp는 상단에서 완전 투명으로 끝난다. emitter 위치는 수명 동안 변하지 않으며 turbulence를 사용하지 않아 프레임마다 입자가 왕복하거나 떨리지 않는다. 높아진 입자가 조기 culling되지 않도록 `visibility_aabb` 높이를 220m 이상 보장한다. 손상 자산은 연기와 별도로 `손상` 상태 표식을 노출한다. 런타임 emitter에는 `preprocess`를 사용하지 않고 최대 입자 버퍼를 scene에 미리 두어 피해 연기 생성 시 시뮬레이션이나 재할당이 한 프레임에 몰리지 않게 한다.
