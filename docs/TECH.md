...
고속 발사체의 연기는 활성 emitter의 정상 방출을 유지하면서 이전·현재 월드 위치 사이를 고정 거리로 추가 표본한다. `local_coords = false`인 emitter에 수동 방출할 때는 월드 위치를 그대로 전달한다. 한 경로 표본에는 좁은 반경의 복수 입자를 배치해 체적을 만들고, 바람은 큰 중력 가속도가 아니라 낮은 가속도·제한된 초기속도·감쇠로 표현한다. 아군과 적 미사일 모두 비행 종료 뒤에도 충분한 입자 수와 수명으로 궤적을 남긴다. 추진 표현은 emissive material과 유한 범위 광원을 함께 사용한다. 타격기의 임무 효과는 기체에서 표적까지 이동하는 투사체와 명중 VFX로 표현하되 gameplay 피해 판정은 임무 상태가 계속 소유한다.
