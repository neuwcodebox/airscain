...
`PowerManager`는 가동 중인 에너지 소비 자산의 명목 수요 합계를 제공한다. 에너지 무기 배치 미리보기는 이 값과 후보 definition의 수요를 더해 현재 수요와 배치 후 수요를 발전 용량과 비교한다. 전력은 전역 capacity 모델이므로 후보와 지원기지 사이에 공간 연결선을 만들지 않는다.
