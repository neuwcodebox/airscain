...
순항체의 지형추종은 진행방향의 제한된 지형 sample에서 안전고도를 정하고 부드럽게 따라가면 충분하다. 종말거리 안에서는 순항 안전고도 강제를 해제하고 남은 이동량이 목표를 지나치면 충돌점에 고정해 임무 판정을 보장한다. 완전한 3D 공력과 최적 경로계획을 요구하지 않는다.
...
요격체는 유도 상실과 비행시간 초과 때 현재 위치에서 자폭한다. 대응책 기만이 성공하면 즉시 폭발하지 않고 플레어·채프의 고정된 허위 반사 위치로 유도된 뒤, 해당 지점을 지나 거리가 다시 증가할 때 유도 이탈로 종료한다. 근접신관이 작동하면 표적의 파괴 여부와 무관하게 요격체 폭발을 생성하므로 내구도가 높은 항공 표적에 비치명 피해만 준 경우에도 투사체가 단순 소실되지 않는다. 공중폭발의 연기·파편 초기속도는 전방향으로 분포시키고 중력은 이후 낙하에만 작게 작용한다.

`ThreatRegistry`는 등록된 위협이 제거될 때 신호를 보낸다. 요격탄은 항적의 분류·추정 위치·불확실성으로 제거된 위협과 상관하고, 일치하면 다음 이동 전에 `표적 격추` 또는 `표적 소실`로 자폭한다. 실체가 없는 항적은 계속 교전할 수 있지만 요격탄이 유도점 45m 안까지 접근한 뒤 거리가 다시 증가하면 `유도 이탈`로 자폭해 고정점을 반복 선회하지 않는다. 상관 및 최소 접근 거리 상태는 진행 중 요격탄 저장에 포함한다.
