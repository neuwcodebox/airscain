...
HUD 시간 제어의 네 `Button`은 toggle 상태를 현재 `GameSession.simulation_speed`에서 역으로 갱신한다. `statistics_changed`마다 0·1·2·4 중 일치하는 버튼 하나만 눌림 상태와 cyan 강조색을 가지며, 별도 `SpeedLabel`은 두지 않는다. 훈련 자동 정지·재생이나 저장 복원으로 속도가 바뀌어도 같은 갱신 경로를 사용한다.
...
훈련 진입 예고는 깊이 검사와 원근 크기에 영향을 받는 3D 라벨이 아니라 도시에서 진입점까지 이어지는 점선 회랑, 화면 가장자리에 고정한 화살표와 `훈련 표적 진입` 라벨로 표현한다. 화면 밖 항적과 훈련 진입 표식은 우측 패널 너비를 하드코딩하지 않고 다른 방향과 같은 화면 가장자리 여백을 사용한다. 훈련 공격 UAV의 시작점은 도시 동쪽으로 전장 크기의 58% 떨어진 해상이며, 비행 표면을 기준으로 고도를 정한다.
