...
방공망 자산 카탈로그는 감시·추적, 지휘·지원, 미사일 방어, 근접·특수 요격의 역할 섹션으로 묶는다. 카탈로그의 위치와 높이는 고도 프로파일 아래의 독립된 우측 영역으로 제한하고, 콘텐츠가 늘어나도 패널 자체가 위로 확장되지 않도록 내부 목록만 스크롤한다. 제목 버튼으로 본문을 접으면 우측 하단에는 한 줄 헤더만 남는다.
...
훈련 진입 예고는 깊이 검사와 원근 크기에 영향을 받는 3D 라벨이 아니라 도시에서 진입점까지 이어지는 점선 회랑, 화면 안전영역 가장자리에 고정한 화살표와 `훈련 표적 진입` 라벨로 표현한다. 안전영역의 좌우 여백은 고정 패널 폭이 아니라 현재 보이는 훈련 패널의 오른쪽 끝과 자산 카탈로그의 왼쪽 시작점에서 계산해 안내문이 어느 쪽에도 가려지지 않게 한다. 일반 화면 밖 항적은 기존처럼 전체 viewport 가장자리를 사용한다. 훈련은 방어 시작 시 카탈로그를 접고 지원 단계에서 다시 펼친다. 적성 잠정 항적만 있을 때는 자동 재생을 유지하고 확인 항적이 생겼을 때만 선택 단계로 전환해 일시정지한다. `PlacementController`는 지면 ray가 빗나간 좌클릭도 화면 좌표와 함께 선택 계층에 전달하고, `AirscainMain`은 화면 안 3D 표식과 화면 밖 전술 표식의 동일한 투영 위치를 사용해 항적을 고른다. 훈련 공격 UAV의 시작점은 도시 동쪽으로 전장 크기의 58% 떨어진 해상이며, 비행 표면을 기준으로 고도를 정한다.
