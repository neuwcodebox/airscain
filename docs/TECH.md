...
대응책 성공 판정은 표적 정의의 플레어·채프 효과와 요격탄 탐색기 민감도 중 우세한 조합을 선택한다. 표적 뒤에는 해당 대응책 입자만 생성하고 내부 판정 문구는 표시하지 않는다. 채프는 320개의 큰 회전 금속 박편과 140개의 고휘도 반사광 입자를 겹쳐 7초 이상 넓게 퍼지는 구름으로 유지한다. 탄약이 완전히 고갈된 자산의 월드 표식은 한 글자 약어 대신 화면 고정 크기의 `탄약 고갈` 문구를 사용한다.

건물과 자산의 피해 연기는 넓은 상자형 방출면에서 부드러운 radial alpha를 가진 billboard card를 고밀도로 내보낸다. `Battlefield`는 생성된 각 건물 transform의 실제 높이로 옥상 anchor를 계산하고, 보호 목표는 기존 고정 Y offset 대신 목표 방향에 가까운 네 옥상을 선택한다. 지붕 바로 위 하부 emitter와 이를 잇는 중부·상부 emitter는 위쪽일수록 더 긴 수명, 높은 상승 속도, 강한 횡풍과 3D noise 난류를 사용한다. 높아진 입자가 조기 culling되지 않도록 층별 `visibility_aabb` 높이를 220–300m로 보장한다. 수명 color ramp는 상단에서 완전 투명으로 끝나며 모든 층이 월드 그림자를 투사한다. 손상 자산은 연기와 별도로 `손상` 상태 표식을 노출한다. 런타임 생성 emitter에는 `preprocess`를 사용하지 않아 피해 연기 생성 시 여러 초 분량의 GPU 시뮬레이션이 한 프레임에 몰리지 않게 한다.
