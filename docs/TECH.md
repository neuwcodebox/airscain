...
훈련 진입 예고는 깊이 검사와 원근 크기에 영향을 받는 3D 라벨이 아니라 도시에서 진입점까지 이어지는 점선 회랑, 화면 안전영역에 고정한 화살표와 `훈련 표적 진입` 라벨로 표현한다. 우측 안전영역은 자산 카탈로그 너비를 제외한다. 훈련 공격 UAV의 시작점은 도시 동쪽으로 전장 크기의 58% 떨어진 해상이며, 비행 표면을 기준으로 고도를 정한다.
