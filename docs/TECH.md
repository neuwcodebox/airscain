...
미사일 연기 emitter는 발사체 종료 시 월드 효과 부모로 재부모화하고 방출을 멈춘다. 이후 인스턴스별 재질 알파를 8초 동안 `smoothstep`으로 1에서 0까지 연속 감쇠하고, 완전 투명 상태를 한 프레임 이상 유지한 뒤 정리한다. 도시 연기는 기능 손실 비율의 단계 수로 재구성하며, 자산 상태 표식과 손상 연기는 공개된 탄약·가동·integrity 상태만 읽는다.
