...
전술 화면 오버레이는 카메라 basis에서 월드 N/E/S/W 방향을 화면 벡터로 변환해 회전 대응 나침반을 그린다. 훈련 진입 예고는 깊이 검사와 원근 크기에 영향을 받는 3D 라벨이 아니라 도시에서 진입점까지 이어지는 점선 회랑, 화면 안전영역에 고정한 화살표와 `E` 라벨로 표현한다. 우측 안전영역은 자산 카탈로그 너비를 제외한다. 훈련 공격 UAV의 시작점은 도시 동쪽으로 전장 크기의 58% 떨어진 해상이며, 비행 표면을 기준으로 고도를 정한다.
...
건물 피해 연기는 점 방출의 수직 원뿔을 사용하지 않는다. 넓은 상자형 방출면에서 수명이 다른 입자를 고밀도로 내보내고, 작은 횡풍과 3D noise 난류, 완전 투명으로 끝나는 color ramp를 조합해 불규칙하게 상승하고 퍼지는 기둥을 만든다. 도시의 여러 피해 지점은 작은 연기 덩어리를 겹쳐 사용하며 개별 입자가 거대한 구로 읽히지 않게 한다.
