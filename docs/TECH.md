...
방공망 자산 카탈로그는 감시·추적, 지휘·지원, 미사일 방어, 근접·특수 요격의 역할 섹션으로 묶는다. 카탈로그의 위치와 높이는 고도 프로파일 아래의 독립된 우측 영역으로 제한하고, 콘텐츠가 늘어나도 패널 자체가 위로 확장되지 않도록 내부 목록만 스크롤한다.
...
훈련 진입 예고는 깊이 검사와 원근 크기에 영향을 받는 3D 라벨이 아니라 도시에서 진입점까지 이어지는 점선 회랑, 화면 안전영역에 고정한 화살표와 `훈련 표적 진입` 라벨로 표현한다. 화면 밖 항적과 훈련 진입 표식의 우측 안전영역은 고도 프로파일과 자산 카탈로그 너비를 제외한다. 훈련 공격 UAV의 시작점은 도시 동쪽으로 전장 크기의 58% 떨어진 해상이며, 비행 표면을 기준으로 고도를 정한다.
...
미사일 연기 emitter는 발사체 종료 시 월드 효과 부모로 재부모화하고 방출을 멈춘다. 이후 인스턴스별 재질 알파를 8초 동안 `smoothstep`으로 1에서 0까지 연속 감쇠하고, 완전 투명 상태를 한 프레임 이상 유지한 뒤 정리한다. 도시 연기는 기능 손실 비율의 단계 수로 재구성하며, 자산 상태 표식과 손상 연기는 공개된 탄약·가동·integrity 상태만 읽는다. 모든 `DefenseUnit`은 상태 표식과 별도로 고정 화면 크기의 역할 아이콘을 유지한다. 감시·추적은 `◎`, 지휘·중계는 `◆`, 무장은 `▲`, 지원은 `■`로 표시하고 색도 역할별로 구분한다.
