...
건물 피해 연기는 점 방출의 수직 원뿔을 사용하지 않는다. 넓은 상자형 방출면에서 수명이 다른 입자를 고밀도로 내보내고, 작은 횡풍과 3D noise 난류, 완전 투명으로 끝나는 color ramp를 조합한다. 하부·중간·상부 emitter를 높이별로 겹쳐 연속적으로 상승하는 기둥을 만들고 모든 층이 월드 그림자를 투사한다. 도시의 피해 지점은 옥상 높이에서 시작하며 개별 입자가 거대한 구로 읽히지 않게 한다. 런타임 생성 emitter에는 `preprocess`를 사용하지 않아 피해 연기 생성 시 여러 초 분량의 GPU 시뮬레이션이 한 프레임에 몰리지 않게 한다.
