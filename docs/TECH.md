...
건물과 자산의 피해 연기는 실제 타격점의 작은 상자형 방출면에서 부드러운 radial alpha billboard를 연속적으로 내보낸다. 도시 plume은 1,500개 버퍼와 18초 수명, 9~13m/s 초기 상승 속도, 아래쪽 중력 가속도와 수평 풍가속도를 사용한다. 따라서 입자는 높아질수록 상승이 감속되고 횡풍 변위는 누적되며, 좁은 확산각과 불투명도가 높은 color ramp가 하부 밀도를 유지한다. emitter 위치는 수명 동안 변하지 않으며 turbulence를 사용하지 않아 프레임마다 입자가 왕복하거나 떨리지 않는다. 높아진 입자가 조기 culling되지 않도록 `visibility_aabb` 높이를 220m 이상 보장한다. 화면용 billboard의 그림자 proxy는 평면 복제가 아닌 저해상도 `SphereMesh`를 사용해 시점이나 렌더러와 무관하게 사각 카드 모서리가 투영되지 않으며, 화면 emitter의 입자 수와 수명을 매 프레임 동기화한다. 손상 자산은 연기와 별도로 `손상` 상태 표식을 노출한다. 런타임 emitter에는 `preprocess`를 사용하지 않고 최대 입자 버퍼를 scene에 미리 두어 피해 연기 생성 시 시뮬레이션이나 재할당이 한 프레임에 몰리지 않게 한다.
