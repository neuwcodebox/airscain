...
미사일 연기 emitter는 발사체 종료 시 월드 효과 부모로 재부모화하고 방출을 멈춘다. 이후 인스턴스별 재질 알파를 16초 동안 `smoothstep`으로 1에서 0까지 연속 감쇠하고, 완전 투명 상태를 한 프레임 이상 유지한 뒤 정리한다. 요격탄·순항미사일·타격기·공대지 탄의 입자 수명과 수량도 기존의 두 배로 함께 높여 감쇠 도중 입자 자체가 먼저 고갈되거나 밀도가 낮아지지 않게 한다. 도시 손상 연기 anchor는 건물 index에 따라 중앙 구조물을 피한 지붕 면 또는 높이 58%·76% 지점의 실제 외벽 면에 둔다. anchor와 별도로 원래 건물 높이를 보존해 낮은 외벽 발화점도 같은 건물 규모에 맞는 plume 높이를 사용한다. 모든 emitter는 그 표면 anchor에 고정하며, 중·상부 입자 자체의 초기 상승 속도와 양의 Y 부력을 건물 높이에 비례해 높인다. `local_coords = false`인 입자는 손상 지점부터 이어진 월드 공간 plume을 남기고 허공의 분리된 발화점이나 움직이는 방출점을 만들지 않는다. 도시 연기는 기능 손실 비율의 단계 수로 재구성하며, 자산 상태 표식과 손상 연기는 공개된 탄약·가동·integrity 상태만 읽는다. 모든 `DefenseUnit`은 상태 표식보다 높은 위치에 작은 고정 화면 크기의 역할 아이콘을 유지한다. 감시·추적은 `◎`, 지휘·중계는 `◆`, 무장은 `▲`, 지원은 `■`로 표시하고 색도 역할별로 구분한다.
