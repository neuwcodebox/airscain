...
고속 발사체의 연기는 frame당 한 지점에서 방출하지 않고 이전·현재 월드 위치 사이를 고정 거리로 표본한다. 추진 표현은 emissive material과 유한 범위 광원을 함께 사용한다. 타격기의 임무 효과는 기체에서 표적까지 이동하는 투사체와 명중 VFX로 표현하되 gameplay 피해 판정은 임무 상태가 계속 소유한다.
