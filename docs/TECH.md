...
대응책 성공 판정은 표적 정의의 플레어·채프 효과와 요격탄 탐색기 민감도 중 우세한 조합을 선택한다. 표적 뒤에는 해당 대응책 입자만 생성하고 내부 판정 문구는 표시하지 않는다. 채프는 얇은 회전 금속 박편을 주체로 하고 소수의 작은 반사 섬광만 보강해 흰 분말 구름처럼 보이지 않으면서 6초 이상 넓게 퍼진다. 탄약이 완전히 고갈된 자산의 월드 표식은 한 글자 약어 대신 화면 고정 크기의 `탄약 고갈` 문구를 사용한다.
