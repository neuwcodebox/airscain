...
섬의 해안 mask는 단순 방사 거리가 아니라 seed에서 유도한 3중·5중 방향 굴곡과 저주파 noise로 방향별 반경을 바꾼다. 지도 가장자리에는 별도 해수 감쇠를 적용해 가장 긴 곶도 전장 경계를 넘지 않게 한다. 도시 중심부는 방사 거리 mask로 부드럽게 평탄화한다. 시가지 필지는 중앙 업무지구를 항상 포함하되 seed에서 유도한 방향별 경계 굴곡 안에서만 후보가 되며, 외곽 후보는 해수면 고도와 경사 제한을 통과해야 한다. 각 활성 필지에만 아스팔트 도로 tile과 보도 블록을 만들고 인접 필지 사이에만 중앙선을 이어, 하나의 정사각 아스팔트 판이 생기지 않게 한다. 건물 크기와 높이는 중심 가중치로 조절하고 외곽 필지에는 여러 저층 동을 배치하며, 빈 필지는 공원이나 주차장으로 채운다.
