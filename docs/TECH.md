...
건물과 자산의 피해 연기는 넓은 상자형 방출면에서 부드러운 radial alpha를 가진 billboard card를 고밀도로 내보낸다. 하부·중간·상부 emitter는 서로 겹치는 높이에서 시작하며 위쪽일수록 더 긴 수명, 높은 상승 속도, 강한 횡풍과 3D noise 난류를 사용한다. 수명 color ramp는 상단에서 완전 투명으로 끝나며 모든 층이 월드 그림자를 투사한다. 도시의 피해 지점은 옥상 높이에서 시작하고 손상 자산은 연기와 별도로 `손상` 상태 표식을 노출한다. 런타임 생성 emitter에는 `preprocess`를 사용하지 않아 피해 연기 생성 시 여러 초 분량의 GPU 시뮬레이션이 한 프레임에 몰리지 않게 한다.
