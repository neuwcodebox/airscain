...
`ThreatRegistry`는 등록된 위협이 제거될 때 신호를 보낸다. 요격탄은 항적의 분류·추정 위치·불확실성으로 제거된 위협과 상관하고, 일치하면 이동 전에 대체 공개 적성 항적을 재획득한다. 즉시 재획득하지 못하면 2.5초 유예 상태로 들어가 현재 속도와 방향으로 연기 궤적을 이어가며 매 tick 후보를 다시 평가한다. 유예 안에 적합한 항적이 확인되면 정상 유도로 복귀하고, 끝까지 없을 때만 보존한 `표적 격추`·`표적 소실`·`유도 상실` 원인으로 자폭한다. 재표적화 시 기존 교전 예약은 해제하고 새 항적 예약으로 바꾼다. 실체가 없는 항적은 계속 교전할 수 있지만 요격탄이 유도점 45m 안까지 접근한 뒤 거리가 다시 증가하면 `유도 이탈`로 자폭해 고정점을 반복 선회하지 않는다. 탄종 적합도, 상관, 최소 접근 거리와 재획득 잔여시간·원인은 진행 중 요격탄 저장·복원에 이어진다.
