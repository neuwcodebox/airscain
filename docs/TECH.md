...
요격체는 유도 상실, 비행시간 초과, 대응책 기만 때 현재 위치에서 자폭한다. 근접신관이 작동하면 표적의 파괴 여부와 무관하게 요격체 폭발을 생성하므로 내구도가 높은 항공 표적에 비치명 피해만 준 경우에도 투사체가 단순 소실되지 않는다. 공중폭발의 연기·파편 초기속도는 전방향으로 분포시키고 중력은 이후 낙하에만 작게 작용한다.
...
대응책 성공 판정은 표적 정의의 플레어·채프 효과와 요격탄 탐색기 민감도 중 우세한 조합을 원인으로 선택한다. 표적 위치에는 해당 대응책 입자와 `플레어 기만` 또는 `채프 기만` 문구를 생성하고, 요격탄 위치에는 같은 원인의 실패 효과와 공중폭발을 생성한다. 탄약이 완전히 고갈된 자산의 월드 표식은 한 글자 약어 대신 화면 고정 크기의 `탄약 고갈` 문구를 사용한다.
