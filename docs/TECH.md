...
도시와 시설 부지는 별도 mask로 부드럽게 평탄화한다. 도시 시각은 아스팔트 기반 위에 보도 블록을 올려 그 간격을 도로로 만들고, 블록 경계의 중앙선으로 격자를 강조한다. 건물 크기와 높이는 중심 가중치로 조절해 업무 중심부에서 외곽으로 낮아지는 스카이라인을 만들며, 빈 블록은 seed에 따라 공원이나 주차장으로 채운다.
