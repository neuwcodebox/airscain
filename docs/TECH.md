...
카탈로그의 휠 이벤트는 `ScrollContainer`의 GUI 입력에서 소비하며 스크롤 경계에서도 상위 viewport로 강제 전달하지 않는다. 카메라는 메인에서 등록한 UI 화면 사각형 안의 휠 입력을 추가로 거부한다. 카메라 방위각은 Q/E와 우클릭 수평 드래그로 바꾸고, 키보드 이동과 중클릭 이동 벡터는 현재 방위각을 적용해 화면 방향을 유지한다.
...
고속 발사체의 연기는 활성 emitter의 정상 방출을 유지하면서 이전·현재 월드 위치 사이를 고정 거리로 추가 표본한다. `local_coords = false`인 emitter에 수동 방출할 때는 월드 위치를 그대로 전달한다. 한 경로 표본에는 좁은 반경의 복수 입자를 배치해 체적을 만들고, 바람은 작은 방향 편향 위에 위치 기반 3D noise 난류를 더해 지그재그 흐름으로 표현한다. 수명 편차는 process material에서 제어하며, color ramp의 alpha가 수명 끝보다 먼저 완전히 사라지도록 해 입자가 갑자기 제거되는 인상을 막는다. 아군과 적 미사일 모두 비행 종료 뒤에도 충분한 입자 수와 수명으로 궤적을 남긴다. 추진 표현은 emissive material과 유한 범위 광원을 함께 사용한다. 타격기의 임무 효과는 기체에서 표적까지 이동하는 투사체와 명중 VFX로 표현하되 gameplay 피해 판정은 임무 상태가 계속 소유한다.
