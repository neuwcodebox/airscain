...
미사일 연기 emitter는 발사체 종료 시 월드 효과 부모로 재부모화하고 방출을 멈춘 뒤 기존 입자의 수명만큼 독립적으로 잔류한다. 도시 연기는 기능 손실 비율의 단계 수로 재구성하며, 자산 상태 표식과 손상 연기는 공개된 탄약·가동·integrity 상태만 읽는다.

고속 발사체의 연기는 활성 emitter의 정상 방출을 유지하면서 이전·현재 월드 위치 사이를 고정 거리로 추가 표본한다. `local_coords = false`인 emitter에 수동 방출할 때는 월드 위치를 그대로 전달한다. 한 경로 표본에는 좁은 반경의 복수 입자를 배치해 체적을 만들고, 바람은 작은 방향 편향 위에 위치 기반 3D noise 난류를 더해 지그재그 흐름으로 표현한다. 수명 편차는 process material에서 제어한다. color ramp는 정규화 수명 45%부터 점진적으로 감쇠하고 88%에 alpha 0에 도달한 뒤 삭제 시점까지 완전 투명한 안전구간을 유지한다. 아군과 적 미사일 모두 비행 종료 뒤에도 충분한 입자 수와 수명으로 궤적을 남긴다. 추진 표현은 emissive material과 유한 범위 광원을 함께 사용한다. 타격기의 임무 효과는 기체에서 표적까지 이동하는 투사체와 명중 VFX로 표현하되 gameplay 피해 판정은 임무 상태가 계속 소유한다.
