...
const DEFAULTS := {"master": 1.0, "missile": 1.0, "gun": 1.0, "explosion": 1.0, "alert": 1.0, "ui": 1.0, "pan": 1.0, "rotation": 1.0, "zoom": 1.0, "fullscreen": false}
...
func set_value(key: String, value: Variant) -> void:
	...
	if key == "fullscreen":
		...
		if not (value is float or value is int) or not is_finite(float(value)):
			return
		value = clampf(float(value), 0.0 if AUDIO_BUSES.has(key) else 0.25, 1.0 if AUDIO_BUSES.has(key) else 2.0)
	values[key] = value
...
func load_preferences() -> void:
	...
	for key: String in DEFAULTS:
		...
		if key == "fullscreen":
			...
			values[key] = clampf(float(value), 0.0 if AUDIO_BUSES.has(key) else 0.25, 1.0 if AUDIO_BUSES.has(key) else 2.0)
