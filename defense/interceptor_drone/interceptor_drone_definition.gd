...
@export var maximum_target_speed: float = 80.0
@export_range(0.0, 1.0) var small_target_match: float = 0.15
@export_range(0.0, 1.0) var missile_target_match: float = 0.25
...
func validation_error() -> String:
	...
	if not base_error.is_empty():
		return base_error
	if attack_range <= 0.0 or maximum_target_speed <= 0.0 or small_target_match < 0.0 or small_target_match > 1.0 or missile_target_match < 0.0 or missile_target_match > 1.0 or launch_interval <= 0.0 or drone_speed <= 0.0 or drone_turn_rate_degrees <= 0.0 or drone_endurance <= 0.0 or drone_damage <= 0.0 or proximity_radius <= 0.0 or drone_count < 1 or engagement_channels < 1 or engagement_channels > drone_count or recharge_duration <= 0.0 or c2_range <= 0.0:
