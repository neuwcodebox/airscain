...
func validation_error() -> String:
	if id.is_empty() or display_name.is_empty() or interceptor_speed <= 0.0 or interceptor_turn_rate_degrees <= 0.0 or interceptor_lifetime <= 0.0 or interceptor_damage <= 0.0 or proximity_radius <= 0.0 or infrared_sensitivity < 0.0 or infrared_sensitivity > 1.0 or radar_sensitivity < 0.0 or radar_sensitivity > 1.0 or minimum_preferred_speed < 0.0 or other_target_match < 0.0 or other_target_match > 1.0 or small_target_match < 0.0 or small_target_match > 1.0 or magazine_capacity < 1 or reserve_ammunition < 0 or reload_duration <= 0.0 or resupply_cost < 0 or resupply_work <= 0.0:
