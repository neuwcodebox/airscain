...
func validation_error() -> String:
	...
	if not base_error.is_empty():
		return base_error
	if attack_range <= 0.0 or fire_interval <= 0.0 or interceptor_speed <= 0.0 or interceptor_turn_rate_degrees <= 0.0 or interceptor_lifetime <= 0.0 or interceptor_damage <= 0.0 or proximity_radius <= 0.0 or infrared_sensitivity < 0.0 or infrared_sensitivity > 1.0 or radar_sensitivity < 0.0 or radar_sensitivity > 1.0 or c2_range <= 0.0 or small_target_match < 0.0 or small_target_match > 1.0 or magazine_capacity < 1 or reserve_ammunition < 0 or reload_duration <= 0.0 or resupply_cost < 0 or resupply_work <= 0.0:
