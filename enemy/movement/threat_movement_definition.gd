...
enum Mode { ALTITUDE_HOLD, TERRAIN_FOLLOWING }
...
func validation_error() -> String:
	if speed <= 0.0 or cruise_altitude <= 0.0 or terminal_distance <= 0.0 or maximum_turn_rate_degrees <= 0.0 or maximum_climb_rate <= 0.0 or terrain_lookahead < 0.0:
