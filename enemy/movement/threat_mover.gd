...
const BALLISTIC_BOOST_SECONDS := 1.5
const BALLISTIC_STEP_SECONDS := 0.05
const BALLISTIC_HORIZONTAL_ACCELERATION := 220.0

enum BallisticStage { BOOST, MIDCOURSE, REENTRY }
...
var ballistic_stage: BallisticStage = BallisticStage.BOOST
var ballistic_acceleration: Vector3 = Vector3.ZERO
var ballistic_stage_seconds: float = 0.0
...
func setup(profile_value: ThreatMovementDefinition, battlefield_value: Battlefield, initial_direction: Vector3) -> void:
	...
	ballistic_stage = BallisticStage.BOOST
	ballistic_stage_seconds = 0.0
	ballistic_acceleration = Vector3.ZERO
...
func capture_state() -> Dictionary:
	return {"velocity": SaveDocument.vector3_to_data(velocity), "ballistic_origin": SaveDocument.vector3_to_data(ballistic_origin), "ballistic_target": SaveDocument.vector3_to_data(ballistic_target), "ballistic_initialized": ballistic_initialized, "ballistic_stage": ballistic_stage, "ballistic_acceleration": SaveDocument.vector3_to_data(ballistic_acceleration), "ballistic_stage_seconds": ballistic_stage_seconds}
...
func restore_state(state: Dictionary, profile_value: ThreatMovementDefinition, battlefield_value: Battlefield) -> void:
	...
	if state.has("ballistic_stage"):
		ballistic_stage = int(state.ballistic_stage) as BallisticStage
		ballistic_acceleration = SaveDocument.vector3_from_data(state.get("ballistic_acceleration", []))
		ballistic_stage_seconds = float(state.get("ballistic_stage_seconds", 0.0))
	else:
		# Older saves stored a sampled arc. Continue from their saved position and velocity.
		ballistic_stage = BallisticStage.MIDCOURSE
		ballistic_acceleration = Vector3.ZERO
		ballistic_stage_seconds = 0.0

func _advance_ballistic(unit: Node3D, body: Node3D, target: Vector3, speed_multiplier: float, delta: float) -> void:
	if ballistic_initialized and ballistic_stage == BallisticStage.REENTRY and ballistic_stage_seconds <= 0.00001:
		return
	if not ballistic_initialized:
		...
		var gravity := profile.ballistic_gravity
		var boost_speed := -gravity * BALLISTIC_BOOST_SECONDS * 0.5 + sqrt(gravity * gravity * BALLISTIC_BOOST_SECONDS * BALLISTIC_BOOST_SECONDS * 0.25 + 2.0 * gravity * profile.ballistic_apex)
		velocity = Vector3.ZERO
		ballistic_acceleration = Vector3(0.0, minf(profile.maximum_climb_rate, boost_speed) / BALLISTIC_BOOST_SECONDS, 0.0)
		ballistic_stage = BallisticStage.BOOST
		ballistic_stage_seconds = BALLISTIC_BOOST_SECONDS
		ballistic_initialized = true
	if ballistic_stage == BallisticStage.BOOST and ballistic_stage_seconds <= 0.00001:
		ballistic_stage = BallisticStage.MIDCOURSE
	var remaining_delta := delta
	while remaining_delta > 0.00001:
		var step := minf(remaining_delta, BALLISTIC_STEP_SECONDS)
		if ballistic_stage == BallisticStage.BOOST:
			step = minf(step, ballistic_stage_seconds)
		elif ballistic_stage == BallisticStage.MIDCOURSE:
			var horizontal := Vector2(ballistic_target.x - unit.global_position.x, ballistic_target.z - unit.global_position.z)
			if horizontal.length() <= maxf(profile.ballistic_reentry_distance, profile.terminal_distance):
				_begin_ballistic_reentry(unit)
				continue
			var desired := Vector3(horizontal.x, 0.0, horizontal.y).normalized() * profile.speed * speed_multiplier
			var horizontal_velocity := Vector3(velocity.x, 0.0, velocity.z).move_toward(desired, BALLISTIC_HORIZONTAL_ACCELERATION * step)
			ballistic_acceleration = Vector3((horizontal_velocity.x - velocity.x) / step, -profile.ballistic_gravity, (horizontal_velocity.z - velocity.z) / step)
		else:
			step = minf(step, ballistic_stage_seconds)
		var previous_velocity := velocity
		unit.global_position += previous_velocity * step + ballistic_acceleration * (0.5 * step * step)
		velocity += ballistic_acceleration * step
		remaining_delta -= step
		if ballistic_stage == BallisticStage.BOOST or ballistic_stage == BallisticStage.REENTRY:
			ballistic_stage_seconds = maxf(0.0, ballistic_stage_seconds - step)
			if ballistic_stage == BallisticStage.BOOST and ballistic_stage_seconds <= 0.00001:
				ballistic_stage = BallisticStage.MIDCOURSE
			elif ballistic_stage == BallisticStage.REENTRY and ballistic_stage_seconds <= 0.00001:
				unit.global_position = ballistic_target
				break
	if velocity.length_squared() > 0.001:
		var up := Vector3.FORWARD if absf(velocity.normalized().dot(Vector3.UP)) > 0.99 else Vector3.UP
		body.look_at(unit.global_position + velocity.normalized(), up)

func _begin_ballistic_reentry(unit: Node3D) -> void:
	var horizontal := Vector2(ballistic_target.x - unit.global_position.x, ballistic_target.z - unit.global_position.z)
	var horizontal_speed := Vector2(velocity.x, velocity.z).length()
	var seconds := clampf(2.0 * horizontal.length() / maxf(horizontal_speed, 1.0), 1.0, 5.0)
	ballistic_acceleration = 2.0 * (ballistic_target - unit.global_position - velocity * seconds) / (seconds * seconds)
	ballistic_stage_seconds = seconds
	ballistic_stage = BallisticStage.REENTRY

func ballistic_phase() -> StringName:
	if not ballistic_initialized or ballistic_stage == BallisticStage.BOOST:
		return &"boost"
	if ballistic_stage == BallisticStage.MIDCOURSE:
		return &"midcourse"
	return &"reentry"
