...
test('stale ended events cannot stop a resumed source and completion is delivered once', async () => {
    const f = fixture();
    const voice = await f.play();
...
test('installation is idempotent and rejects incompatible engine interfaces', async () => {
    const f = fixture();
...
test('pause or stop in the initial play call cannot leak a source start', async () => {
    const f = fixture();
    const paused = f.create(2, 4);
    paused.pause(true);
    await Promise.resolve();
    assert.equal(f.nodes[0].offset, undefined);
    near(paused.getPlaybackPosition(), 2);
    paused.pause(false);
    ...
    stopped.clear();
    await Promise.resolve();
    assert.equal(f.nodes[2].offset, undefined);
