...
func test_placement_and_selection_share_c2_and_support_relations() -> void:
	...
	assert_eq(_metric_value(main.hud.asset_metrics, "전체 수요 / 공급"), "12 / 20")
	assert_eq(_metric_value(main.hud.asset_metrics, "이 자산 전력 수요"), "12")
	assert_eq(_metric_value(main.hud.asset_metrics, "전력망 상태"), "정상")
	var support_definition := main.scenario.available_defenses[5]
