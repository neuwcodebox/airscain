...
func test_training_lesson_progresses_through_deployment_support_and_recovery() -> void:
	...
	assert_eq(training.training_controller.step, TrainingController.Step.WAIT_RESUPPLY)
	assert_string_contains(training.hud.training_body.text, supply_description)
	...
	assert_eq(training.support_manager.task_detail_status(battery), "수리 진행 · 3.0초")
	training.hud.training_next_button.pressed.emit()
	...
	training.support_manager.gameplay_tick(2.9)
	assert_lt(battery.integrity, battery.definition.maximum_integrity)
	training.support_manager.gameplay_tick(0.2)
	assert_eq(training.training_controller.step, TrainingController.Step.WAIT_REPAIR)
	assert_string_contains(training.hud.training_body.text, repair_description)
	assert_true(training.hud.training_next_button.visible)
	training._process(10.0)
	assert_eq(training.training_controller.step, TrainingController.Step.WAIT_REPAIR)
	training.hud.training_next_button.pressed.emit()
...
func test_combat_audio_catalog_and_clip_selection_match_content() -> void:
	...
	assert_eq(CombatAudio.all_streams().size(), 32)
...
func test_long_range_launcher_exposes_munition_mode_control() -> void:
	...
	assert_eq(option.get_item_text(1), "일반 요격탄")
	assert_eq(option.get_item_text(2), "고속 요격탄")
	assert_not_null(option.get_item_icon(1))
	...
	assert_eq(rows[1].label, "일반 요격탄")
	assert_eq(rows[1].tooltip, option.get_item_tooltip(1))
	assert_same(rows[1].icon, option.get_item_icon(1))
...
func test_ballistic_missile_reenters_near_target_with_steep_terminal_velocity() -> void:
	...
	var target := main.objective.global_position
	for flight_distance: float in [3000.0, 9000.0]:
		var origin := target + Vector3(flight_distance, 20.0, 0.0)
		var unit := Node3D.new()
		var body := Node3D.new()
		unit.add_child(body)
...
func test_asset_icons_status_badges_and_reload_bars_share_hover_target() -> void:
	...
	assert_eq(TacticalScreenOverlay.asset_hint_statuses(battery), ["재보급 대기"])
	...
	assert_eq(TacticalScreenOverlay.asset_hint_statuses(battery), ["기능 정지", "재보급 대기"])
	battery.complete_repair()
	...
	assert_has(TacticalScreenOverlay.asset_hint_statuses(battery), "재장전 · 4.0초")
	battery.magazine.reload_remaining = 0.0
	battery.magazine.rounds = 1
	assert_true(TacticalScreenOverlay.asset_hint_statuses(battery).is_empty())
...
func test_selected_weapon_requests_resupply_from_limited_support_capacity() -> void:
	...
	gun.set_automatic_resupply(true)
	main.support_manager.restore_state(main.support_manager.capture_state())
	...
	gun.magazine.reserve = 0
	main.support_manager.gameplay_tick(1.1)
	assert_eq(main.support_manager.task_status(gun), "재보급 진행")
	gun.set_automatic_resupply(false)
	main.support_manager.restore_state(main.support_manager.capture_state())
	main.support_manager.gameplay_tick(3.0)
	assert_eq(gun.magazine.reserve, gun.magazine.reserve_capacity)
