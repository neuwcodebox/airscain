...
func build(generator: WorldGenerator, blocks: Array[Dictionary]) -> void:
	...
	var farm_noise := _noise(generator.seed_value ^ 0x6A1D, 0.0011, 1)
	...
	for z_index: int in TEXELS:
		for x_index: int in TEXELS:
			...
			var land := smoothstep(generator.sea_level + 3.0, generator.sea_level + SHORE_HEIGHT + 3.0, height)
			var clear := 1.0 - city_mask[index]
			var dry := clampf(dry_noise.get_noise_2d(x, z) * 0.9 + 0.45 - bend * 0.35 + smoothstep(35.0, 80.0, height) * 0.25, 0.0, 1.0)
			dryness[index] = dry
			var near_city := _district_edge_distance(Vector2(x, z), districts)
			var farm_band := smoothstep(40.0, 110.0, near_city) * (1.0 - smoothstep(420.0, 720.0, near_city))
			var farm := smoothstep(-0.25, 0.05, farm_noise.get_noise_2d(x, z)) * farm_band * (1.0 - smoothstep(4.5, 8.0, slope)) * land * clear
			farmland[index] = farm
			var woods := smoothstep(-0.12, 0.28, forest_noise.get_noise_2d(x, z) + bend * 0.25 - dry * 0.25 + smoothstep(8.0, 22.0, slope) * 0.15)
			woods *= land * clear * (1.0 - smoothstep(30.0, 40.0, slope)) * (1.0 - farm)
			forest[index] = woods
