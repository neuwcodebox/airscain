...
func run() -> void:
	...
	if OS.get_cmdline_user_args().has("--capture-identity-icons-only") or OS.get_cmdline_user_args().has("--capture-reload-only"):
		...
		main.set_process(false)
		main.camera_rig.set_process(false)
		var definitions: Array[DefenseDefinition] = []
		for index: int in [8, 0, 7, 1, 3, 4, 6, 9, 10, 2, 5]:
			definitions.append(main.scenario.available_defenses[index])
		...
		for index: int in definitions.size():
			...
			unit.global_position = Vector3(float(index % 6) * 65.0 - 160.0, 0, 350.0 + float(index / 6) * 110.0)
		...
		for frame: int in 6:
			await process_frame
		_save_capture("/tmp/airscain_identity_catalog.png")
		print("IDENTITY_ICONS_CAPTURE_OK all_defenses selected night catalog")
		quit()
		return
...
func _capture_missile_smoke_trail() -> void:
	...
	await _wait_simulation_seconds(fade_quarter)
	...
	if not (aged_opacity > fading_opacity and fading_opacity > near_end_opacity and near_end_opacity > 0.0):
		...
		quit(1)
		return
	await _wait_simulation_seconds(fade_quarter + 0.05)
	...
	if not is_instance_valid(smoke) or smoke.current_opacity_ratio > 0.001 or smoke.release_remaining <= 0.0:
		...
		quit(1)
		return
	await _wait_simulation_seconds(smoke.transparent_cleanup_delay + 0.1)
