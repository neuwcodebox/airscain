...
Windows, Godot on PATH, Python 3.10+ with Pillow, FFmpeg/ffprobe on PATH (the installed `D:/Utils/ffmpeg-master-latest-win64-gpl/bin` fallback is supported). Existing NanumSquareB and Windows Arial Bold are rasterized. The licensed music file is `build/trailer_v7/assets/volatile-reaction.mp3`; see ASSETS.md.

## Reproduce

```powershell
python tools/trailer/produce.py capture intro --lang ko --seconds 17
python tools/trailer/produce.py capture intro --lang en --seconds 17
python tools/trailer/produce.py capture expansion --seconds 6
python tools/trailer/produce.py capture raid --seconds 43.5
...
The missile falls normally until the first blackout. Terminal capture samples at half simulation speed; the editor restores 2x playback before blackout. After blackout, seven successive real moments are held on screen, with advancing source time hidden in the black cuts. Both missiles remain framed. The edit is selected relative to the actual outcome event, never including the outcome. This is intentional editorial stop-motion, not a claim about gameplay pause behavior. Music gain stays constant; no siren or warning caption is added.

## Delivery and checks

Current output: `build/trailer_v7/`. Previous versions remain in their directories.
...
Verification checks real placements, rapid first fire, increasing persistent asset counts, zero later contact sounds, preexisting fleet plus offscreen ballistic launch, genuine ballistic reentry, normal-speed pre-blackout motion and advancing near-impact stills without resolution, exact source ranges and the dynamically computed output frame count. Intentional expansion fades and ending black cards are allowed explicitly. Source trimming excludes the one pre-roll display frame before the first directed frame. Mastering uses one static gain, aiming at −16 LUFS when the −1.5 dBTP peak ceiling permits it; no dynamic normalization. Objective measurements do not establish perceptual audio quality. Actual listening remains separately unverified unless documented.
